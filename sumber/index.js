...
<body>
    ...
    <script>
        async function checkProxyHealth() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    healthStatus.innerHTML = '<i class="fas fa-check-circle"></i> <strong>Online</strong> - ' + result.isp + ' (' + result.country + ') <br>Ping: ' + result.ping + 'ms | IP: ' + result.ip;
                } else {
                    healthStatus.className = 'health-status offline';
                    healthStatus.innerHTML = '<i class="fas fa-times-circle"></i> <strong>Offline</strong> - ' + result.error;
                }
            } catch (error) {
                healthStatus.className = 'health-status offline';
                healthStatus.innerHTML = '<i class="fas fa-exclamation-triangle"></i> <strong>Error</strong> - ' + error.message;
            }
        }
        ...
        function displayResults(config) {
            ...
            vlessResults.innerHTML = '<div class="vless-card"><h4 style="color: white; margin-bottom: 15px;">VLESS Link</h4><div class="vless-config">' + vlessLink + '<button class="copy-btn" onclick="copyToClipboard(\'' + vlessLink + '\', this)"><i class="fas fa-copy"></i> Copy</button></div><h4 style="color: white; margin-bottom: 15px;">JSON Configuration</h4><div class="vless-config">' + JSON.stringify(config, null, 2) + '<button class="copy-btn" onclick="copyToClipboard(JSON.stringify(config, null, 2), this)"><i class="fas fa-copy"></i> Copy</button></div><h4 style="color: white; margin-bottom: 15px;">Configuration Details</h4><div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; color: white;"><div><strong>UUID:</strong> ' + config.uuid + '</div><div><strong>Proxy:</strong> ' + config.proxy + ':' + config.port + '</div><div><strong>Server Name:</strong> ' + config.serverName + '</div><div><strong>Network:</strong> ' + config.network.toUpperCase() + '</div><div><strong>Path:</strong> ' + config.path + '</div><div><strong>Security:</strong> ' + config.security.toUpperCase() + '</div><div><strong>CDN:</strong> ' + (config.cdn ? 'Enabled' : 'Disabled') + '</div><div><strong>Reverse Domain:</strong> ' + (config.reverseDomain ? 'Enabled' : 'Disabled') + '</div></div></div>';
